...
export default function PokeCard({
  ...
  return (
    <div className="relative flex flex-col gap-3 my-10 bg-white border rounded-lg shadow-lg pt-20 ">
      <Link href={`/pokemon/${pokemon.name}`}>
        <img
          className=" w-52 absolute left-1/2 -translate-x-1/2 top-0 -mt-28 aspect-square object-contain"
          src={pokemon.image}
          alt="pokemon image"
        />
      </Link>
      ...
      <Link href={`/pokemon/${pokemon.name}`}>
        <div className=" flex flex-row items-center justify-center gap-3  py-4  mx-auto w-full">
          {types &&
            types.map((type) => {
              return (
                <div
